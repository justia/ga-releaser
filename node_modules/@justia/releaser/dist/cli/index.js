...
const run = async () => {
    ...
    const localInput = await (0, github_1.readWorkflowConfig)();
    const config = (0, github_1.loadInputIntoConfig)(index_1.default, localInput);
    /**
    ...
    const runRelease = (options) => {
        void log_1.default.group('release-it', () => {
            return (0, release_it_1.default)(options)
                .then((output) => {
                if (output) {
    ...
    const isExecutable = !args
        ...
        switch (argument) {
            case 'config':
                return (0, commands_1.configCommand)(option, config);
            case 'calver':
                return (0, commands_1.calverCommand)(option, config);
            case 'increment':
                return (0, commands_1.incrementCommand)(option, config);
            case 'tag':
                return (0, commands_1.tagCommand)(config);
            case 'help':
                return (0, commands_1.helpCommand)();
            case 'debug':
                isDebugEnv = true;
                return true;
            default:
