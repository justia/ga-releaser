...
class ConventionalChangelog extends conventional_changelog_1.default {
    ...
    getIncrementedVersion({ increment, latestVersion, isPreRelease, preReleaseId }) {
        ...
        if (!calverPlugin) {
            return super.getIncrementedVersion({ increment, latestVersion, isPreRelease, preReleaseId });
        }
        const options = calverPlugin in plugins ? plugins[calverPlugin] : {};
        const { namespace, config, log } = this;
        const container = { config, log };
