...
class GitConfigPlugin extends release_it_1.Plugin {
    ...
    registerPrompts() {
        super.registerPrompts({
            ...
            clean: {
                type: 'confirm',
                message: () => 'Do you want to clean the local repository?',
                default: true
            }
        });
    }
    ...
    async afterRelease() {
        ...
        await this.step({
            enabled: true,
            task: () => this.removeRemoteNonexistentTracks(),
            label: 'Git clean local',
            prompt: 'clean'
        });
    }
    /**
    ...
    async removeBranch(branch) {
        ...
        return this.checkoutBranch(baseBranch)
            .then(() => this.exec(['git', 'branch', '--delete', branch]))
            .then(() => this.exec(['git', 'push', 'origin', '--delete', `heads/${branch}`]))
            .catch((err) => {
            this.log.error(`Could not remove the branch ${branch} from origin`);
            this.debug(err);
        });
    }
    async removeRemoteNonexistentTracks() {
        return this.exec(['git', 'fetch', '-p']).catch((err) => {
            this.log.error(`Could not update the local repository tracks`);
            this.debug(err);
        });
    }
}
