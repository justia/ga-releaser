...
const findConfiguration = (yamlContent) => {
    ...
    const releaserStep = Object.values(jobs)
        .reduce((arr, { steps }) => arr.concat(Array.isArray(steps) ? steps : []), [])
        .find((step) => (step.uses.startsWith('justia/ga-releaser') ? step : null));
    return releaserStep ? releaserStep.with : {};
};
...
exports.readWorkflowConfig = async () => {
    ...
    const configuration = findConfiguration(yamlContent);
