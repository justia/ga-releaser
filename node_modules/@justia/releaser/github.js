...
const findConfiguration = (yamlContent) => {
    ...
    const releaserStep = Object.values(jobs)
        ...
        .find((step) => step.uses?.startsWith('justia/ga-releaser'));
    let conf = {};
    if (releaserStep?.with) {
...
const findAuthor = () => {
    ...
    let email;
    try {
        name = (0, utilities_1.exec)('git config user.name').trim();
    }
    catch (e) {
        name = (0, utilities_1.exec)('git log --format="%aN" -1').trim();
    }
    try {
        email = (0, utilities_1.exec)('git config user.email').trim();
    }
    catch (e) {
        email = (0, utilities_1.exec)('git log --format="%aE" -1').trim();
    }
    return { name, email };
};
...
const readWorkflowConfig = async () => {
    ...
    return log_1.default.group(`Loading input values...`, async () => Object.entries(exports.inputDefaults).reduce((obj, [key, value]) => {
        ...
        if (inputValue !== '') {
            finalValue = (0, utilities_1.isBoolean)(value) ? (0, utilities_1.parseInputBoolean)(inputValue, !!finalValue) : inputValue;
        }
...
const loadInputIntoConfig = (defaultConfig, input) => {
    const config = { ...defaultConfig };
    const npm = input['npm-enabled'] !== false && { publish: input['npm-publish'] };
    (0, utilities_1.mergeDeep)(config, {
        git: {
            commitMessage: input['git-message'],
            requireCommits: input['require-commits'],
            requireUpstream: input['require-upstream'],
            tagMatch: input['tag-match']
        },
        github: {
        ...
        plugins: {
            '@justia/releaser/plugins/conventional-changelog.js': {
                preset: input.preset,
                infile: input['output-file'] === 'false' ? false : input['output-file']
            },
    ...
    if (input['versioning-specification'] === 'calver') {
        config.plugins = {
            '@justia/releaser/plugins/calver.js': {
                format: input['calver-format']
            },
            ...config.plugins
        };
    }
    const { inputs: { increment: payloadIncrement } = {} } = payload;
    const increment = input.increment || payloadIncrement;
    if (increment) {
        (0, utilities_1.mergeDeep)(config, { increment });
    }
    return config;
};
exports.loadInputIntoConfig = loadInputIntoConfig;
